...
describe Bosh::Director::PackageCompiler do
  ...
  it "doesn't do anything if there are no packages to compile" do
    ...
    check_event_log do |events|
      ...
      expect(events.map { |e| e['stage'] }.uniq).to eql(['Preparing package compilation'])
      expect(events.map { |e| e['task'] }.uniq).to eql(['Finding packages to compile'])
      expect(events.map { |e| e['index'] }.uniq).to eql([1])
      expect(events.map { |e| e['total'] }.uniq).to eql([1])
