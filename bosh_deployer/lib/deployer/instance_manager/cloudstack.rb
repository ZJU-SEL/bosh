  ...
  class InstanceManager
    ...
    class Cloudstack < InstanceManager
      ...
      def configure
        ...
        @ssh_user = properties["cloudstack"]["ssh_user"] || "vcap"
      ...
      def discover_bosh_ip
        if exists?
          ...
          floating_ip = cloud.cloudstack.ipaddresses.find {
      ...
      def disk_size(cid)
        # CloudStack stores disk size in Byte but we work with MiB
        (cloud.cloudstack.volumes.get(cid).size / (1024 * 1024 )).ceil
      end

      def persistent_disk_changed?
        # since CloudStack stores disk size in Byte and we use MiB there
